...
2026-09-25, cloud session, design only. Noodle's request is `CARD-POOL.md` **B34**, verbatim. Nothing in the
...
`../relics/OUTFITS-LIST.md`, `../Archive/demo1/rework/cards/CARD-POOL-01.md`.
...
3. **The card list, character by character**, Cinder first, then Clemence, then the four that work. Each
   ...
   `../Archive/demo1/rework/cards/CARD-POOL-01.md` §4, with a `From` column so nothing is rebuilt that already works.
...
6. **Rates and numbers last**, then an All the Crunch run before and after (`../tooling/`).
7. **Cut cards retire** through `honeycomb.retiredCardArray` with definitions in
   a new retired-cards file in `../Archive/demo1/Archive/`, so saves survive.
