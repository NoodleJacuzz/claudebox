...
This is a standing brief. Do not add progress to it. Progress goes in `CATCH-UP.md`, guesses go in
`INFERENCES.md`, Noodle's words are in `FEEDBACK.md`.

| File in this folder | Holds |
|---|---|
| `BRIEF.md` | this file: what to build, in order, and the check each step ends on |
| `CATCH-UP.md` | where the build is. Update it as each step lands |
| `FEEDBACK.md` | Noodle's requests, verbatim. The open items |
| `INFERENCES.md` | choices made without Noodle, and how to change each one |
...
- `tools/draft-sim/` never plays a fight. It prices cards from a hand-written table. It is staying
  where it is, untouched, for later (see `FEEDBACK.md` T5). Do not use it and do not delete it.
...
9. Update `CATCH-UP.md` when each step lands, not at the end. A session can be stopped at any moment.
...
**How to measure.** Run this before and after, and record both times in `CATCH-UP.md`:
...
`../performance/FEEDBACK.md` as a finding. Do not claim it fixes anything players reported.
...
bot looks the way it does. Noodle wants **one skill level, a skilful player** (`FEEDBACK.md` T3), so fix
...
Measure the cost of each of those two changes in seconds per fight and write it in `CATCH-UP.md`. If the
...
**Check:** full default run, wall-clock time recorded in `CATCH-UP.md` at `--workers 1` and
...
Try these in order, and write down in `CATCH-UP.md` which one was used for each row above:
...
- Measure runs per minute on one worker and write it in `CATCH-UP.md`. **Every time estimate below
...
Noodle's direction (`FEEDBACK.md` T4, T5): there is no synergy table. Card synergy is subjective and
...
His words are in `FEEDBACK.md` T4. In short: a bot built around "which card should I pick" tends to miss
...
- **A section called "When the bot stops picking cards".** For each stage of a run: how often the
  ...
  warning that deckbuilding is ending early, and name the stage where it drops. (`INFERENCES.md` I2.)
...
- **Party each run:** `INFERENCES.md` I4. Every character has their own EXP, so the rule decides how
...
- **The gauntlet:** read `../chessmaster/STATUS.md` for the exact conditions before writing this. What
