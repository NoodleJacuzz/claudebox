...
**Chosen session 44** rather than asking, because `CATCH-UP.md` already said numbers are out of scope
for this folder.
...
**To pivot:** if `../rework/enemies/` decides Lust should be worth less than damage point-for-point,
the conversions are the thing to revisit and the ratios are all in `RECAST-01.md` §E3.
...
`CATCH-UP.md` said *"every enemy should be able to answer which Lust tag it teaches"*. That is a
...
discovery rates — but it is a large swing and it is flagged in `_archive/FEEDBACK-DONE.md` E10 for
Noodle to look at.
