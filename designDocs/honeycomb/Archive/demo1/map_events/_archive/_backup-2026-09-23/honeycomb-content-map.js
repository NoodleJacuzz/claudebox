...
honeycomb.regionArray = [
	{
		...
		//THE BOSS POOL (session 34): a run meets one of these, rolled when the map is generated (see
		...
		//THIS BOSS MOVED UP (E7 Stage 3). Noodle's pairing is one Act1-1 boss to one route: this slot
		//leads to Act1-A, the Head Gardener to Act1-B, the Matriarch to Act1-C. The E12 recast (session
		//50) replaced the clerk who held it with THE SHROUD, a mold colony grown into a reaper's shape --
		//so the reason for the pairing changed with him: Act1-A is the spread at its thickest, and a
		//mold that has built itself a body is that sub-act stated outright.
		//THIS IS A RELOCATION, which this workstream's own law forbids -- Noodle authorised it in the
		//E7 brief. It was re-budgeted to this region's boss row on the way; see the table entry.
		...
		//SESSION 17 DOUBLED THE FLOORS: a run was ending before a build could get going. The map now
		//scrolls sideways (see honeycomb-map.js), and a longer floor is a wider map at the same node
		//spacing, so this is the one number that lengthens it.
		rowCount: 18,
	},
	{
		//THE NAME IS NOODLE'S, SESSION 55: "Mushroom Frontier is still called The Flooded Vault".
		//`enemy_overhaul/` recast this region's five enemies as dry frontier myconids under arms in
		//session 34 and renamed twelve of its encounters off the water theme; the region kept the old
		//title, so the place and the things standing in it disagreed outright. See `map/FEEDBACK.md` B31.
		//
		...
		//so they wait for Noodle's eye rather than being picked here.
		index: "floodedVault",
		...
		//Noodle's painting for this route's fights (session 57), read by honeycomb.combatScene.backdropPath.
		battleBackdropPath: "backgrounds/frontier",
		colorNear: "#1b2f3a",
		colorFar: "#0e1720",
		//THE RUN'S TWO BOSSES ARE THE TWO REGIONS' BOSSES (session 16): the Upper Catacombs end at the
		...
		//The Act1-1 boss now called The Shroud moved up out of here (E7 Stage 3), leaving the Juggernaut
		...
		backdropArray: [
			{
				...
				//same shape, so an anchor stays on its landmark. SESSION 17: a 16:9 painting cannot be
			...
			{
				//THE PROOF (round 02, item 20): a map no procedural layout could produce. From the Watch
				...
				//SESSION 17 doubled its anchors; the backward climb is in the positions, and the edges are
				//derived by proximity now.
				index: "vaultShowcase",
	...
	//THE OTHER TWO SUB-ACTS (E7). Act1-1 splits three ways and these are the second and third of the
	...
	//reordering these two silently repoints every encounter that names them.
	{
		...
		colorNear: "#2f3a1f",
		colorFar: "#16220f",
		//THE SISTERS IN THE ARBOR (E7 Stage 2). Both bodies, one encounter, no victory condition -- see
	...
	},
	{
		...
		colorNear: "#3a3324",
		colorFar: "#1e1a12",
		//THE END OF THE ROAD (E7 Stage 2). One body -- the moth-taur and the torso on it are the same
	...
	//No `layoutIndex`: it falls through to the procedural default and needs no painting.
	{
		...
		backgroundPath: "map/background-vault",
		//Every fight in the gallery stands in front of Noodle's chess hall (session 57), read by
		...
		//A CORRIDOR, NOT A MAP (Noodle, session 47): "Narrow it down to a straight line of just 4
		//encounters. Celestial normal, Infernal normal, rest, Infernal elite, rest, Celestial elite,
		//which includes Anastasia." Six rows, one node each, no choice anywhere -- a detour the devoted
		//player walks once, not a second act. The `line` layout reads this plan and rolls nothing.
		layoutIndex: "line",
		rowPlanArray: [
...
honeycomb.eventArray = [
	...
	//-------------------------------------------------------------------------------------------
	//Rest. An ordinary event, run by the rest node type.
	//
	//Worth reading as an example: healing, upgrading and removing a card used to be three hand-written
	//branches of a bespoke screen. All three are effect lists here, and the two that need the player
	//to pick a card use `chooseCards`, which stops resolution, asks, and continues.
	//-------------------------------------------------------------------------------------------
	{
		index: "theCampfire",
		name: "The Campfire",
		...
		//One picture per character: whoever leads the party is the one sitting at the fire (2026-09-23).
		imagePath: "events/campfire-{leader}",
		backgroundPath: "events/backdrop-campfire",
		//Rest is always available; it is not drawn from the event pool.
		weight: 0,
		choiceArray: [
			{
				text: "Sleep",
				previewText: "Every member of the party heals 30% of their maximum health.",
				effectArray: [{ index: "heal", amount: { index: "math", operation: "divide",
					left: { index: "stat", stat: "maxHealth", of: "target" }, right: 3 },
					targetOverride: "allAllies" }],
				resultText: "The fire burns down while you sleep. Everyone stands up straighter for it.",
			},
			//The same deck services the shop's counter runs (honeycomb.deckServiceArray), in the
			//campfire's own words.
			{
				text: "Sharpen a card",
	...
	//-------------------------------------------------------------------------------------------
	{
		index: "theQuietSpring",
		name: "The Quiet Spring",
		appearsWhenBroken: true,
		weight: 55,
		text: "Water comes out of the rock too warm to be groundwater and too clean to be anything else. " +
			"Whoever is having the worst of it can barely be talked past the edge.",
		imagePath: "map/event-spring",
		choiceArray: [
			{
				index: "bathe",
				discovery: true,
				text: "Let them stay in as long as they want.",
				previewText: "Removes 18 Lust from the whole party. The day runs long.",
				effectArray: [
					{ index: "soothe", amount: 18, targetOverride: "allAllies" },
					{ index: "gainResource", resource: "gold", amount: -20 },
				],
				resultText: "They come out steadier. Something in the water took the coins out of your purse, " +
					"or you paid somebody, or you left them; nobody can quite account for it afterwards.",
			},
			{
				text: "A quick rinse and move on.",
				previewText: "Removes 8 Lust from the whole party.",
				effectArray: [{ index: "soothe", amount: 8, targetOverride: "allAllies" }],
				resultText: "It helps. It is not enough, and everyone knows it is not enough.",
			},
			{
				text: "Fill every skin you have and go.",
				previewText: "The whole party gains 2 Composure for the next fight.",
				effectArray: [{ index: "applyStatus", status: "composure", stacks: 2, targetOverride: "allAllies" }],
				resultText: "Carrying it turns out to be worth more than sitting in it.",
			},
		],
	},
	{
		...
		appearsWhenBroken: true,
		weight: 40,
		text: "Something in the tunnel has been waiting for exactly this: a party with somebody in it who " +
			"cannot argue. It offers, in a voice like a poultice, to take the worst of it away.",
		imagePath: "map/event-hands",
		choiceArray: [
			{
				index: "accept",
				discovery: true,
				...
				resultText: "It is gentle, and it is thorough, and something goes into the deck with you when " +
					"you leave.",
			},
			{
				index: "refuse",
				discovery: true,
				text: "Put yourself between it and them.",
				...
				resultText: "It does not like being refused. Everyone is cut about the arms, and everyone is " +
	...
	},
	{
		...
		text: "A dry well, lit from the bottom by something soft and yellow. A rope ladder has been nailed to the inside, and the nails are new.",
		imagePath: "map/event-waxlight",
		weight: 30,
		choiceArray: [
			{
				index: "climb",
			...
			},
			{
				...
				resultText: "Whatever was down there had been paying somebody. The nails were worth prying out.",
			},
			{
				text: "Leave it alone.",
				...
				effectArray: [],
				resultText: "The light is still there when you look back. It is a little closer to the top.",
			},
		],
	},
	{
		index: "theTollBench",
		name: "The Toll Bench",
		text: "A stone bench across the corridor, worn smooth in three places. A slot in the armrest is the exact width of a coin.",
		imagePath: "map/event-bench",
		weight: 30,
		choiceArray: [
			{
				index: "payToll",
				...
				resultText: "The bench takes the coin without a sound. Sitting on it is the best any of you have felt in days.",
			},
			{
				index: "sitFree",
				discovery: true,
				...
				resultText: "You find 25 gold in the slot. You also find that you cannot stop thinking about the bench.",
			},
			{
				text: "Walk around it.",
	...
	},
	{
		...
		text: "A room someone has been tending. Rows of caps in graded sizes, a watering can, and a chalk tally on the wall that stops abruptly at forty-one.",
		imagePath: "map/event-garden",
		weight: 25,
		choiceArray: [
			{
				index: "harvest",
			...
			},
			{
				...
				resultText: "The smoke is worse than expected. Under the third row, a strongbox nobody had gotten around to moving.",
			},
			{
				...
				resultText: "Forty-two. Something slides a key under the door on your way out.",
			},
		],
	},
	{
		index: "theCardsharp",
		name: "The Cardsharp",
		text: "A figure at a folding table, dealing to nobody. It does not look up. \"One trade,\" it says. \"Something of yours for something of mine. You will not get to choose which.\"",
		imagePath: "map/event-cardsharp",
		weight: 22,
		choiceArray: [
			{
				index: "tradeCard",
			...
			},
			{
				...
				resultText: "\"Generous,\" it says, and deals two cards face down. They are both worth having.",
			},
			{
				text: "Refuse.",
				previewText: "Nothing happens. Your deck is left alone.",
				effectArray: [],
				resultText: "It keeps dealing. You are fairly sure it was already playing your hand.",
			},
		],
	},
	{
		...
		text: "A door of black iron, bolted from this side. Something behind it is breathing slowly, and has been for a long time.",
		imagePath: "map/event-door",
		weight: 20,
		choiceArray: [
			{
				index: "force",
				...
				resultText: "The bolts come back one at a time, and the breathing stops.",
			},
			{
				index: "listen",
				...
				effectArray: [],
				resultText: "The breathing follows you down the corridor for a while, then gives up.",
			},
		],
		pageArray: [
			{
				...
				index: "beyond",
				text: "Past the door is a reliquary nobody has opened, and a long drop into the dark beside it.",
				choiceArray: [
					{
						index: "reliquary",
						discovery: true,
	...
	},
	{
		...
		text: "An alcove with a shallow bowl and a great many small offerings. Buttons, mostly. A few teeth.",
		imagePath: "map/event-shrine",
		weight: 25,
		choiceArray: [
			{
				index: "offering",
				...
				//A RANDOM relic the party does not already carry (round 03, item 7). It named Crackseal Wax
				//before, which the Sealed Door also gives, so the second one paid for nothing.
				effectArray: [
			...
			},
			{
				...
				resultText: "Seventy gold in buttons and teeth. The alcove is much colder on the way out than it was on the way in.",
			},
		],
	},
];
...
//---------------------------------------------------------------------------------------------------
//Treatments for the Broken state (session 5)
//---------------------------------------------------------------------------------------------------
//> This makes not being near a rest site less obscenely punishing.
//
...
//---------------------------------------------------------------------------------------------------
//Events and the Broken state (session 5)
//---------------------------------------------------------------------------------------------------
//> allow for events to have variants if a broken character is present, and for events to appear if a
//> character is broken, and for the shop to have a special sale if a character is broken to play an
//> event healing them. This makes not being near a rest site less obscenely punishing.
//
...
honeycomb.eligibleEventArray = function () {
	...
	if (result.length === 0) return honeycomb.rollableEventArray();
	return result;
};
...
//which cards qualify. Until round 04 (item 3) the shop used an older picker that hid every STARTER
//card, so a Strike could be left at a campfire but never sold off. Reached through the `deckService`
//effect, which may override the question's wording.
honeycomb.deckServiceArray = [
	{
		index: "removeCard",
		description: "Choose a card and remove it from your deck for good.",
		effectArray: [{
			//removableOnly keeps a curse that "cannot be removed by ordinary means" off the offer (item 14).
	...
	},
	{
		...
		effectArray: [{
			//upgradeableOnly also shows each card's upgraded version before it is committed (item 6).
...
//Named once because the Lust it removes is the same amount (session 57).
...
//The shop's own numbers. `brokenOfferArray` (session 5) is the SPECIAL SALE: while anybody in the
...
honeycomb.shopTuning = {
	...
	//Card ranks are Starter / Common / Rare (round 07). Equipment and relics keep their own three ranks.
	cardPriceArray: {
		starter: 30,
		common: 55,
		rare: 120,
	...
	//No `uncommon` row: the tier was retired in session 55c and nothing carries it. See
