...
Progress goes in `TOOLING.md`; the questions it leaves for Noodle are `TOOLING.md` Q1–Q8; guesses a
build session makes go in an inferences file (INFERENCES.md) that this folder does not have yet.

| File in this folder | Holds |
|---|---|
| `QUALITY-LAB-BRIEF.md` | this file: the pitch verbatim, what the engine does today, the design, the build order |
| `TOOLING.md` | where the work is. Update it as each phase lands |
| `TOOLING.md` | Noodle's words, and the eight decisions the design waits on |

Read `../BASICS.md` first. The Battle Lab this builds on is `../Archive/demo1/Archive/FEEDBACK-07.md` §A2 and
`../engine/ENGINE.md` A10; the animation vocabulary is `../Archive/demo1/Archive/POLISH-01.md` §3–4.
...
TU:312), so their hit is CSS only, which is `../art_pipeline/ART-PIPELINE.md`'s unbuilt tilt-and-redden.
...
11. **Comments are lean and quote nobody.** His words are in this folder's `TOOLING.md`.
...
drift guard block, and the template table handed to `../card_pool/` as the field a new card sets.
...
- **`../engine/` B16, the top demo goal.** Per-beat `scheduledAt → paintedAt` stamps and a human's
...
- **`../art_pipeline/` B13 and `../art_pipeline/` B21.** A vfx picker with anchor and offset, the at-once
  sweep, and the flurry scaling are the assignment half of the VFX workstream, on real timing.
- **`../engine/` B9–B11.** The sfx grid replaces searching the table, `impactMsMap` is the number the
  library has been missing, and P1 ends the two-sources problem.
- **`../engine/` B6, the hand.** The same recorder and stamps apply to the hand's feel once the cycle can
