...
6. **`[act1-flora]` and `[forestDay]` are not real shortcuts.** They read as placeholders in these
   ...
   session 49 while building `PROMPTS-NETTLE-02.txt`, which writes them out as `cave, vines, nature`
   and `outdoors, forest, grass, trees, daytime, sunlight`. `[inn]` has the same status and already
   had the same treatment in `PROMPTS-NETTLE-01.txt`. Every sketch above uses at least one of them,
